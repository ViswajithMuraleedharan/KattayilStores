...
import { MdVisibility, MdClose, MdSearch } from "react-icons/md";
import { orders as initialOrders } from "../../data/dummy";
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {filtered.map((o) => (
                  <tr key={o.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                    <td className="px-4 py-3 font-semibold text-blue-600 dark:text-blue-400">{o.id}</td>
                    <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">{o.customer}</td>
                    <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{o.phone}</td>
                    <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{o.items.length} item(s)</td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <div className="flex items-center justify-between mb-5">
                <h3 className="font-bold text-slate-900 dark:text-white">Order — {selected.id}</h3>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                {[["Customer", selected.customer], ["Phone", selected.phone], ["Date", selected.date], ["Total", `₹${selected.total}`]].map(([k, v]) => (
