    ...
    <motion.div whileHover={{ y: -4 }} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden group">
      <Link to={`/shop/${product.id}`}>
        <div className="aspect-square overflow-hidden bg-slate-100 dark:bg-slate-700">
          <img src={product.imageUrl ?? product.image} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        </div>
      </Link>
      <div className="p-4">
        ...
        <div className="flex items-center gap-1 mb-3">
          ...
          <span className="text-xs text-slate-400">({product.reviewCount ?? product.reviews})</span>
        </div>
